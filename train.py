...
parser.add_argument('--data_root', default='./CIFAR10', type=str)
...
parser.add_argument('--param_share', default=False, type=bool,
...
def main():
    ...
    # load pre-trained teacher params
    # with open(os.path.join('/root/data/outputs/haoran/rocket/resnet_40_1_booster', opt.teacher_id, 'log.txt'),
    #           'r') as ff:
    #     line = ff.readline()
    ...
    #     os.path.join('/root/data/outputs/haoran/rocket/resnet_40_1_booster', opt.teacher_id, 'model.pt7'))
    # params_t = model_data['params']
    # stats_t = model_data['stats']
    #
    # params_t = flatten_params(params_t)
    # for k, v in params_s.iteritems():
    #     if 'block0' in k or 'block1'in k:
    #     #if 'conv0' in k:
    #         ks = k
    #         params_s[ks] = params_t[ks.replace('groups','group')]

    # deal with teacher
    if opt.teacher_id != '':
        with open(os.path.join('/root/data/outputs/haoran/rocket', opt.teacher_id, 'log.txt'), 'r') as ff:
            line = ff.readline()
            r = line.find('json_stats')
            info = json.loads(line[r + 12:])
        f_t = resnet(info['depth'], info['width'], num_classes)[0]
        model_data = torch.load(os.path.join(
            '/root/data/outputs/haoran/rocket', opt.teacher_id, 'model.pt7'))
        params_t = model_data['params']
        stats_t = model_data['stats']
        ...
        def f(inputs, params, stats, mode):
            if opt.gamma:
                ...
                return y_s, y_t_auto, y_t, [at_loss(x, y) for x, y in zip(g_s, g_t)]
            else:
    ...
    if opt.gamma:
        ...
        classacc_s_top1 = tnt.meter.ClassErrorMeter(topk=[1])
    else:
    ...
    def h(sample):
        ...
        if opt.teacher_id != '':
            if opt.gamma:
                ys, y_t_auto, y_t, loss_groups = data_parallel(f, inputs, params, stats, sample[2], np.arange(opt.ngpu))[:4]
                ...
                loss_course = opt.gamma * ((y_t_auto - ys) * (y_t_auto - ys)).sum() / opt.batchSize
                ...
                return opt.beta * sum(loss_groups) + F.cross_entropy(y_t_auto, targets) + F.cross_entropy(ys, targets) + opt.gamma * ((y_tech_temp - ys) * (
    ...
    if opt.gamma:
        def on_forward(state):
            ...
            classacc_s_top1.add(state['output'][0].data, torch.LongTensor(state['sample'][1]))
        def on_start_epoch(state):
            classacc_s.reset()
            classacc_t.reset()
            classacc_s_top1.reset()
            meter_loss.reset()
            meter_loss_s.reset()
            meter_loss_t.reset()
            meter_loss_c.reset()
        ...
        def on_end_epoch(state):
            ...
            test_acc_s_top1 = classacc_s_top1.value()[0]
            print log({
